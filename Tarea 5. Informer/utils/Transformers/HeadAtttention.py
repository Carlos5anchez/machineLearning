...
class MultiHeadAttention:
    ...
    def scaled_dot_product_attention(self, queries, keys, values):
        tranpuesta=keys.swapaxes(-1, -2)  # transponer la matriz de claves  (swapaxes(-1, -2) 
        matmul_qk = np.matmul(queries, tranpuesta) / np.sqrt(self.depth) # Producto punto entre las consultas y las claves 
