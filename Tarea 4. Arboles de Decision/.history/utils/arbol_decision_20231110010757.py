...
class DecisionTreeClassifier:
    ...
    def _best_split(self, X, y):
        ...
        for column in range(numCol): # Recorremos las columnas
            thresholds = np.unique(X[:, column]) # Obtenemos los valores únicos de la columna, Son todas mis posibles condiciones
            for threshold in thresholds: # Recorremos los valores únicos
              ...
              # Por cada valor único, separamos los datos en dos grupos, los que cumplen la condición y los que no
                left_indices = X[:, column] < threshold # Obtenemos los indices de la izquierda
                right_indices = X[:, column] >= threshold # Obtenemos los indices de la derecha
                ...
                if gini < best_gini: # Si el gini es menor al mejor gini
                    ...
                    best_feature = column # Actualizamos el mejor feature
