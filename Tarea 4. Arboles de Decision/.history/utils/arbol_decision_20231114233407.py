...
class DecisionTreeClassifier:
    ...
    def _crecer_arbol(self, X, y, depth=0):
        numClases = [np.sum(y == i) for i in np.unique(y)] # Muestra el numero de clases
        predicted_class = np.argmax(numClases) # Obtenemos la clase con mayor número de muestras
        ...
        if depth >= self.max_depth:
            return node
        # Detenemos la división si solo hay una clase
