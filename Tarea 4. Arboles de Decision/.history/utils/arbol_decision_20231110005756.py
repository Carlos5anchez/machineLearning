...
class DecisionTreeClassifier:
    ...
    def _grow_tree(self, X, y, depth=0):
        ...
        best_feature, best_threshold, best_gini = self._best_split(X, y) # Obtenemos el mejor split
        ...
        left = self._grow_tree(X[left_indices], y[left_indices], depth + 1) # Creamos el nodo de decisión izquierdo
        right = self._grow_tree(X[right_indices], y[right_indices], depth + 1) # Creamos el nodo de decisión derecho
    ...
    def _best_split(self, X, y):
        ...
        for feature_index in range(numCol): # Recorremos las columnas
            thresholds = np.unique(X[:, feature_index]) # Obtenemos los valores únicos de la columna, Son todas mis posibles condiciones
            for threshold in thresholds: # Recorremos los valores únicos
                left_indices = X[:, feature_index] < threshold # Obtenemos los indices de la izquierda
                right_indices = X[:, feature_index] >= threshold # Obtenemos los indices de la derecha
                ...
                if gini < best_gini: # Si el gini es menor al mejor gini
                    best_gini = gini # Actualizamos el mejor gini
                    best_feature = feature_index # Actualizamos el mejor feature
