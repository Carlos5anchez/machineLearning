    ...
    def _init_positional_encoding(self):
        ...
        for pos in range(self.max_seq_len): #Filas
            for i in range(0, self.d_model, 2): #Columnas
                pos_encoding[pos, i] = np.sin(pos / (10000 ** ((2 * i)/self.d_model))) #Columnas pares (COSENO)
                pos_encoding[pos, i + 1] = np.cos(pos / (10000 ** ((2 * (i + 1))/self.d_model))) #Columnas impares (SENO)
